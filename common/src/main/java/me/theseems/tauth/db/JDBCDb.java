...
public class JDBCDb implements AuthDB {
    ...
    private void init() {
        try {
            ...
            PreparedStatement st
                    = connection.prepareStatement("CREATE TABLE IF NOT EXISTS TAuth (uuid VARCHAR(40) PRIMARY KEY UNIQUE, hash VARCHAR(512), ip VARCHAR(20), expire timestamp)");
            st.execute();
            connection.close();
    ...
    public void setHash(UUID player, String hash) {
        try {
            initPlayer(player);
            Connection connection = getConnection();
            PreparedStatement statement = connection.prepareStatement("UPDATE tauth SET hash=? WHERE uuid=?");
